...
<html>
   ...
   <!-- end sidebar -->
	...
	<div>
	<p style="font-size: 26pt; margin-top : 10px;" color="white">Transactions</p>
	</div>
	...
	<tbody>
	<?php
	while($row_invoice=mysqli_fetch_assoc($result_trans)){
		
		echo '<tr id="'.$row_invoice['txid'].'"><td>'.$row_invoice['txid'].'</td>
		<td>'.$row_invoice['txdate'].'</td>
		<td>'.$row_invoice['wallet'].'</td>
		<td>'.$row_invoice['amount'].'$</td>
		<td style="text-transform : uppercase;">'.$row_invoice['type'].'</td>
		</tr>';
